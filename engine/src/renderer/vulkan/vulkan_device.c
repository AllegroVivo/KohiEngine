...
typedef struct vulkan_physical_device_requirements {
    ...
    Boolean sampler_anisotropy;
    Boolean discrete_gpu;
...
Boolean select_physical_device(vulkan_context* context);
Boolean physical_device_meets_requirements(
...
Boolean vulkan_device_create(vulkan_context* context) {
    ...
    Boolean present_shares_graphics_queue = context->device.graphics_queue_index == context->device.present_queue_index;
    Boolean transfer_shares_graphics_queue = context->device.graphics_queue_index == context->device.transfer_queue_index;
    ...
    for (UInt32 i = 0; i < index_count; ++i) {
        ...
        if (indices[i] == context->device.graphics_queue_index) {
            queue_create_infos[i].queueCount = 2 - 1;  // WTF WHY WAS THIS LINE CAUSING AN ERROR? Adding '-1' fixes it.
        }
        queue_create_infos[i].flags = 0;
        queue_create_infos[i].pNext = 0;
        Single queue_priority = 1.f;
...
Boolean select_physical_device(vulkan_context* context) {
    ...
    for (UInt32 i = 0; i < physical_device_count; ++i) {
        ...
        Boolean result = physical_device_meets_requirements(
            physical_devices[i],
            context->surface,
            &properties,
            &features,
        ...
        if (result) {
            ...
            for (UInt32 j = 0; j < memory.memoryHeapCount; ++j) {
                Single memory_size_gib = (((Single)memory.memoryHeaps[j].size) / 1024.0f / 1024.0f / 1024.0f);
