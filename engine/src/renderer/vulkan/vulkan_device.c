...
Boolean vulkan_device_detect_depth_format(vulkan_device* device) {
    const UInt64 candidate_count = 3;
    ...
    for (UInt32 i = 0; i < physical_device_count; ++i) {
        ...
        if (result) {
            ...
            for (UInt32 j = 0; j < memory.memoryHeapCount; ++j) {
                Single memory_size_gib = (((Single)memory.memoryHeaps[j].size) / 1024.f / 1024.f / 1024.f);
